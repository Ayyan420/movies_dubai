...
class SaleProductConfiguratorController(Controller):
    ...
    def sale_product_configurator_get_values(
        ...
        **kwargs,
        ...
        stock_per_ptav = {}
        all_ptavs_mains = product_template.valid_product_template_attribute_line_ids.mapped('product_template_value_ids')
        not_warehouse = request.env['stock.warehouse'].sudo().search([], limit=1)
        wh_name = not_warehouse.code or not_warehouse.name or 'WH'
        for ptav in all_ptavs_mains:
            stock_per_ptav[ptav.id] = {
                'total': 0.0,
                'warehouses': {wh_name: 0.0}
            }
        for variant in product_template.product_variant_ids:
            for ptav in variant.product_template_attribute_value_ids:
                if ptav.id not in stock_per_ptav:
                    not_warehouse = request.env['stock.warehouse'].sudo().search([],limit=1)
                    wh_name = not_warehouse.code or not_warehouse.name or 'WH'
                    stock_per_ptav[ptav.id] = {'total': 0.0, 'warehouses': {'%s'%str(wh_name):0}}
                for quant in variant.stock_quant_ids:
                    warehouse = quant.location_id.warehouse_id
                    if warehouse:
                        wh_name = warehouse.code or warehouse.name or 'WH'
                        stock_per_ptav[ptav.id]['warehouses'][wh_name] = stock_per_ptav[ptav.id]['warehouses'].get(wh_name, 0.0) + quant.quantity
                        stock_per_ptav[ptav.id]['total'] += quant.quantity

        formatted_stock = {}
        for ptav_id, data in stock_per_ptav.items():
            if data['warehouses']:
                formatted_stock[ptav_id] = ', '.join(f"{wh} ({qty})" for wh, qty in data['warehouses'].items())
            else:
                formatted_stock[ptav_id] = data['total']
            ...
            products=[
                dict(
                    **self._get_product_information(
                    ...
                    stock_per_ptav=formatted_stock
                )
            ],
            optional_products=[
                dict(
    ...
    def sale_product_configurator_update_combination(
        ...
        **kwargs,
        ...
        ptav_idss = set(ptav_ids or [])
        matched_variants = product_template.product_variant_ids
        ...
        for ptav_id, data in stock_per_ptav.items():
            if data['warehouses']:
                warehouse_info = ', '.join(f"{wh} ({qty})" for wh, qty in data['warehouses'].items())
                formatted_stock[ptav_id] = warehouse_info
            else:
                formatted_stock[ptav_id] = data['total']
        values['stock_per_ptav'] = formatted_stock


        selected_ptav_ids = set(ptav_ids or [])
        stock_per_ptav = {}
        all_ptavs_mains = product_template.valid_product_template_attribute_line_ids.mapped('product_template_value_ids')
        wh_name = request.env['stock.warehouse'].sudo().search([], limit=1).code or 'WH'
        for ptav in all_ptavs_mains:
            stock_per_ptav[ptav.id] = {
                'total': 0.0,
                'warehouses': {wh_name: 0.0}
        ...
        for variant in product_template.product_variant_ids:
            variant_ptav_ids = set(variant.product_template_attribute_value_ids.ids)
            if selected_ptav_ids.issubset(variant_ptav_ids):
                for ptav in variant.product_template_attribute_value_ids:
                    for quant in variant.stock_quant_ids:
                        warehouse = quant.location_id.warehouse_id
                        if warehouse:
                            wh = warehouse.code or warehouse.name or 'WH'
                            if ptav.id not in stock_per_ptav:
                                stock_per_ptav[ptav.id] = {'total': 0.0, 'warehouses': {wh: 0.0}}
                            stock_per_ptav[ptav.id]['warehouses'][wh] = stock_per_ptav[ptav.id]['warehouses'].get(wh, 0.0) + quant.quantity
                            stock_per_ptav[ptav.id]['total'] += quant.quantity
        formatted_stock = {}
        for ptav_id, data in stock_per_ptav.items():
            if data['warehouses']:
                ...
                formatted_stock[ptav_id] = data['total']
        values['stock_per_ptav_only_selected'] = formatted_stock
        print(values)
        return values
