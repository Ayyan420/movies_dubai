...
export class ProductTemplateAttributeLine extends Component {
    ...
    static props = {
        ...
        stock_per_ptav: { type: Object, optional: true },
        stock_per_ptav_only_selected: { type: Object, optional: true },
    };
    ...
    updateSelectedPTAV(event) {
        const clickedPtavId = parseInt(event.target.value);
        const currentSelected = this.props.selected_attribute_value_ids;

        // For single-select (radio)
        if (!this.props.attribute.display_type === 'multi') {
            if (currentSelected.includes(clickedPtavId)) {
                // User clicked the already selected option → unselect
                this.env.updateProductTemplateSelectedPTAV(
                    this.props.productTmplId, this.props.id, null, false
                );
            } else {
                // Normal selection
                this.env.updateProductTemplateSelectedPTAV(
                    this.props.productTmplId, this.props.id, clickedPtavId, false
                );
            }
        } else {
            // multi-select logic (checkbox)
            this.env.updateProductTemplateSelectedPTAV(
                this.props.productTmplId, this.props.id, clickedPtavId, true
            );
        }
    }
    ...
    getPTAVSelectName(ptav) {
        if (ptav.price_extra) {
            ...
            return ptav.name + " (" + sign + " " + price + ")" +" - "+ this.getStockLabel(ptav.id);
        } else {
            return ptav.name +" - "+ this.getStockLabel(ptav.id);
        }
    }
    ...
    getStockLabel(ptavId) {
        if (this.props.stock_per_ptav_only_selected?.[ptavId]) {
            return this.props.stock_per_ptav_only_selected[ptavId];
        }
        if (this.props.stock_per_ptav?.[ptavId]) {
            return this.props.stock_per_ptav[ptavId];
        }
        return '';
    }

    getStockLabelColor(ptavId) {
        if (this.props.stock_per_ptav_only_selected) {
            const sstock = this.props.stock_per_ptav_only_selected[ptavId] || '';
            if (sstock.includes("(0)")) {
                return false;
            } else if (sstock.includes("(0.0)")) {
                return false;
            } else {
                return true;
            }
        }
        else if (this.props.stock_per_ptav) {
            const stock = this.props.stock_per_ptav[ptavId] || '';
            if (stock.includes("(0)")) {
                return false;
            } else if (stock.includes("(0.0)")) {
                return false;
            } else {
                return true;
            }
